...
/**
 * Implement BPR with limited negative sample.
 * Ding, Feng, et all. "An Improved Sampler for Bayesian Personalized 
 * Rankink by Leveraging View Data"
 * 
 * @author guanghuiyu
 *
 */
public class bpr_negativesample extends TopKRecommender {
	/** Model priors to set. */
  ...
  public Integer [][]negdata;
  boolean showProgress;
  public String onlineMode = "u";
  int usercount =0;
  int itemcount =0;
  int negnum = itemCount; // number of negative samples 

  Random rand = new Random();
	public bpr_negativesample(SparseMatrix trainMatrix, ArrayList<Rating> testRatings,
			int topK, int threadNum, int factors, int maxIter, double lr, boolean adaptive, double reg, 
			double init_mean, double init_stdev, boolean showProgress,int showbound ,int showtime,int K ,int negnum) {
	...
	public void buildModel() {	
		...
		negdata = new Integer[userCount][];
		for (int i =0;i<userCount;i++) {
			ArrayList<Integer> itemList = trainMatrix.getRowRef(i).indexList();
			buydata[i] = itemList.toArray(new Integer [itemList.size()]);
			negdata[i] = new Integer [negnum];
			for (int j=0;j<negnum;j++) {
				int b = rand.nextInt(itemCount);
				while (trainMatrix.getValue(i, b) != 0) {
					b = rand.nextInt(itemCount);
				}		
				negdata[i][j] = b;
			}
		}
		System.out.println("array has been available");
		System.out.println("neg sample has been available : ");
	...
	private void update_ui_K(int u, int i) {
		// sample a negative item (uniformly random)
		int j = negdata[u][rand.nextInt(negnum)];
	
        int jsam = 0;
        double jscore = 0;
		for (int k=0; k<paraK; k++){	
			j = negdata[u][rand.nextInt(negnum)];
			while (trainMatrix.getValue(u, j) != 0) {
				j = negdata[u][rand.nextInt(negnum)];
			}
			if (k == 0) {
